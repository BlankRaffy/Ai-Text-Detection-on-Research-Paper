...
#varie versioni per creare i vari file usando i vari metodi di estrazione
'''
...
# Definisci le colonne del DataFrame
columns = ['file', 'abstract', 'introduction', 'conclusion']
df_lda = pd.DataFrame(columns=columns)
...
for file in tqdm.tqdm(os.listdir(original_paper)):
    ...
    # Aggiungi la nuova riga al DataFrame
    df_lda = pd.concat([df_lda, pd.DataFrame([new_row])], ignore_index=True)

# Percorso per il file CSV di output
csv_file_path = 'Topic_Extraction/topic/plagiated_topic_lda.csv'

# Salva il DataFrame nel file CSV
df_lda.to_csv(csv_file_path, index=False)
