...
columns=['original_file','abstract_sentence','intro_sentence','conclusion_sentence']
df_final_result = pd.DataFrame(columns= columns)


original_file_directory ='Dataset/original_paper'
df = pd.read_csv('Ai_Detection/results/most_similar_part_for_file.csv')


for ele in tqdm.tqdm(os.listdir(original_file_directory)):
    original_file_name= original_file_directory+'/'+ele
    ...
    new_row = {'original_file':ele,'abstract_sentence':df_result_abstract,'intro_sentence':df_result_intro,'conclusion_sentence':df_result_conclusion} 
    df_final_result = pd.concat([df_final_result, pd.DataFrame([new_row])], ignore_index=True)



df_final_result.to_csv('AI_Detection/results/Ai_Detection_result_on_original.csv',index=False)
